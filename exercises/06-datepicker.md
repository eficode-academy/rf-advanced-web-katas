...
with SeleniumLibrary, we can still utilize many more libraries in our test cases. The library
...
- Give the keyword the amount of `days` we want to go into the future.
...
Also, we're not sure if we have to click the button in the first place. But first things first:
...
Now that we have our datepicker open, we have to do some extra steps before we can select our date.
First, we need to get today's date. Second, we need to get the date `days` amount of days into the future.
...
- Inside your keyword, get the current date with `Get Current Date` and store it in a variable. Remember
to have `result_format=datetime` as an argument for that keyword call.
- Get a date in the future using `Add Time To Date` using your `days` argument. Store the return value
into a variable. Remember to use `result_format=datetime` again as an argument for the keyword call.
...
- Add a variable `DATEPICKER_NEXT_BUTTON` to your `Variables` table.
...
- Add your `Select Date From Future` call to `Fill All Form Fields` keyword with the argument `3`
to make it get 3 days into the future.
- Add `DEFAULT_DAYS` variable into your `Variables` table.
- Give `Fill All Form Fields` a new argument `days` and give it `DEFAULT_DAYS` as a default value.
- Change your `Select Date From Future` call to use the new `days` argument.
